...
class Tag(DefaultModel):
    name = models.CharField(
        max_length=settings.FIELD_MAX_LENGTH, verbose_name='название тэга'
    )
    color = models.CharField(
        max_length=settings.FIELD_LOW_LENGTH, verbose_name='цветовой hex'
    )
...
class Ingredient(DefaultModel):
    name = models.CharField(
        max_length=settings.FIELD_MAX_LENGTH, verbose_name='название'
    )
    measurement_unit = models.CharField(
        max_length=settings.FIELD_LOW_LENGTH, verbose_name='ед.измерения'
    )
...
class Recipe(DefaultModel):
    ...
    tags = models.ManyToManyField(
        Tag, related_name='tags', verbose_name='тэг', through='RecipeTag'
    )
...
class ShoppingCart(DefaultModel):
    owner = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='shopping_cart_owner'
    )
    recipes = models.ForeignKey(
        Recipe, on_delete=models.CASCADE, related_name='shopping_cart_recipes'
    )
